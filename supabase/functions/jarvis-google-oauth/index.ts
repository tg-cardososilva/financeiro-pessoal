...
async function tokenScopes(accessToken: string, tokenPayload: any) {
  ...
  try {
    ...
    if (r.ok) {
      for (const scope of String(info?.scope || '').split(' ').filter(Boolean)) scopes.add(scope);
    }
  } catch (e) {
...
async function driveMetadataProbe(accessToken: string) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`DRIVE_METADATA_PROBE_${r.status}:${message}`);
  }
...
async function upsertErrorConnection(userId: string, provider: string, info: any, scopes: string[], reason: string) {
  ...
  await admin.from('jarvis_connections').insert({
    user_id: userId,
    provider,
    connected_at: null,
    ...payload,
...
async function oauthCallback(req: Request) {
  if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
    return redirectWith({ jarvis_google: 'error', reason: 'google_not_configured' });
  }
  ...
  if (stateError || !stateRow || !ALLOWED_PROVIDERS.has(provider)) {
    return redirectWith({ jarvis_google: 'error', reason: 'invalid_state' });
  }
  ...
  if (provider === DRIVE_PROVIDER) {
    if (!grantedScopes.includes(DRIVE_SCOPE)) {
      await upsertErrorConnection(userId, provider, info, storedScopes, 'drive_scope_missing');
      await markStateUsed(state);
      return providerRedirect(provider, 'error', 'drive_scope_missing');
    }
    if (!refreshToken) {
  ...
  const connectionPayload = {
    ...
    metadata: {
      ...
      oauth_checkpoint: provider === DRIVE_PROVIDER
        ? { ok: true, drive_metadata_readonly: true, checked_at: new Date().toISOString() }
  ...
  if (existing) {
    ...
    const { data, error } = await admin.from('jarvis_connections').insert({
      user_id: userId,
      provider,
      ...connectionPayload,
    }).select('*').single();
